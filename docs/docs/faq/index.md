...
Hermit currently works with `bash`, `zsh`, `fish` natively.

For `fish` shell, when using hermit in a repo created prior to native fish support, run `hermit init` again and check in
the new `activate-hermit.fish` script.
